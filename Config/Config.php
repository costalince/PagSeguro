 ...
 * These preferences can come from the default config file (PagSeguro_Library/config/PagSeguroConfig.php).
*/
class PagSeguro_Config_Config{
	...
	private function __construct() {
		...
		require_once PagSeguro_Library::getPath().DIRECTORY_SEPARATOR."Config".DIRECTORY_SEPARATOR."Ini.php";
	...
	public static function init() {
		if (self::$config == null) {
			self::$config = new PagSeguro_Config_Config();
		}
		return self::$config;
	}
	...
	public static function getAccountCredentials() {
		if (isset(self::$data['credentials']) && isset(self::$data['credentials']['email']) && isset(self::$data['credentials']['token'])) {
			return new PagSeguro_Domain_AccountCredentials(self::$data['credentials']['email'], self::$data['credentials']['token']);
		} else {
