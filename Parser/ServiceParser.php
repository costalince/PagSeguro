...
class PagSeguro_Parser_ServiceParser{
	...
	public static function readErrors($str_xml) {
		...
		if (isset($data['error']) && is_array($data['error'])) {
			if (isset($data['error']['code']) && isset($data['error']['message'])) {
				array_push($errors, new PagSeguro_Domain_Error($data['error']['code'], $data['error']['message']));
			} else {
				foreach($data['error'] as $key => $value) {
					if (isset($value['code']) && isset($value['message'])) {
						array_push($errors, new PagSeguro_Domain_Error($value['code'], $value['message']));
					}
				}				
			}
		}
