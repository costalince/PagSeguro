...
class PagSeguro_Parser_TransactionParser extends PagSeguro_Parser_ServiceParser {
	
	public static function readSearchResult($str_xml) {
		
		$parser = new PagSeguro_Utils_XmlParser($str_xml);
		$data = $parser->getResult('transactionSearchResult');
		
		$searchResutlt = new PagSeguro_Domain_TransactionSearchResult();
	...
	public static function readTransaction($str_xml) {
		...
		$transaction = new PagSeguro_Domain_Transaction();
		...
		// <transaction> <type>
		if (isset($data["type"])) {
			$transaction->setType(new PagSeguro_Domain_TransactionType($data["type"]));
		}
		
		// <transaction> <status>
		if (isset($data["status"])) {
			$transaction->setStatus(new PagSeguro_Domain_TransactionStatus($data["status"]));
		}
		
		if (isset($data["paymentMethod"]) && is_array($data["paymentMethod"])) {
			
			// <transaction> <paymentMethod>
			$paymentMethod = new PagSeguro_Domain_PaymentMethod();
			
			// <transaction> <paymentMethod> <type>
			if (isset($data["paymentMethod"]['type'])) {
				$paymentMethod->setType(new PagSeguro_Domain_PaymentMethodType($data["paymentMethod"]['type']));
			}
			
			// <transaction> <paymentMethod> <code>
			if (isset($data["paymentMethod"]['code'])) {
				$paymentMethod->setCode(new PagSeguro_Domain_PaymentMethodCode($data["paymentMethod"]['code']));
			}				
		...
		if (isset($data["sender"])) {
			...
			if (isset($data["sender"]["phone"])) {
				...
				$phone = new PagSeguro_Domain_Phone();
		...
		if (isset($data["shipping"]) && is_array($data["shipping"])) {
			...
			if (isset($data["shipping"]["type"])) {
				$shipping->setType(new PagSeguro_Domain_ShippingType($data["shipping"]["type"]));
			}
	...
	private static function parseTransactionSummary($data) {
		...
		$transactionSummary = new PagSeguro_Domain_TransactionSummary();
		
		if (isset($data['type'])){
			$transactionSummary->setType(new PagSeguro_Domain_TransactionType($data['type']));
		}
		if (isset($data['code'])){
		...
		if (isset($data['status'])){
			$transactionSummary->setStatus(new PagSeguro_Domain_TransactionStatus($data['status']));
		}
		if (isset($data['netAmount'])){
