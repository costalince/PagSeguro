...
class PagSeguro_Parser_PaymentParser extends PagSeguro_Parser_ServiceParser {
	...
	public static function getData($payment) {
		...
		if (count($items) > 0) {
			...
			foreach ($items as $key => $value) {
				...
				if ($items[$key]->getAmount() != null) {
					$amount = PagSeguro_Helper_Helper::decimalFormat($items[$key]->getAmount());
				...
				if ($items[$key]->getShippingCost() != null) {
					$data["itemShippingCost$i"] = PagSeguro_Helper_Helper::decimalFormat($items[$key]->getShippingCost());
				}
			}
		...
		if ($payment->getExtraAmount() != null) {
			$data['extraAmount'] = PagSeguro_Helper_Helper::decimalFormat($payment->getExtraAmount());
		}
	...
	public static function readSuccessXml($str_xml) {
		...
		$PaymentParserData = new PagSeguro_Parser_PaymentParserData();
