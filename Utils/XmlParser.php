...
class PagSeguro_Utils_XmlParser {
	...
	public function __construct($xml) {
		...
		if (!xml_parse($parser, $xml)) {
			throw new Exception ("PagSeguro_Library XML parsing error: (". xml_get_error_code($parser) .") " . xml_error_string (xml_get_error_code($parser)));
		} else {
	...
	public function getResult($node = null) {
		...
		if ($node) {
			if (isset($result[$node])) {
				...
				throw new Exception ("PagSeguro_Library XML parsing error: undefined index [$node]");
			}
		} else {
			return $result;
		}
